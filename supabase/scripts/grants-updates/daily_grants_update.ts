...
async function main() {
  ...
  try {
    ...
    // Import distributed lock manager for coordination
    const { distributedLockManager } = await import('../../src/services/sync/DistributedLockManager');
    
    // Use distributed lock to prevent concurrent daily updates
    const result = await distributedLockManager.withLock(
      'daily_grants_update', 
      async () => {
        console.log('🔒 Distributed lock acquired for daily grants update');
        
        // Run API updates sequentially to prevent database conflicts
        // Each API source gets its own lock internally
        const results = await Promise.allSettled([
          distributedLockManager.withLock('api_sync_grants_gov', updateGrantsGov),
          distributedLockManager.withLock('api_sync_federal_register', updateFederalRegister),
          distributedLockManager.withLock('api_sync_california_grants', updateCaliforniaGrants),
          distributedLockManager.withLock('api_sync_world_bank', updateWorldBank),
          distributedLockManager.withLock('api_sync_nsf', updateNSF)
        ]);
        
        // Log results
        results.forEach((result, index) => {
          const sources = ['GrantsGov', 'FederalRegister', 'CaliforniaGrants', 'WorldBank', 'NSF'];
          if (result.status === 'fulfilled' && result.value !== null) {
            console.log(`✅ ${sources[index]} sync completed successfully`);
          } else if (result.status === 'fulfilled' && result.value === null) {
            console.log(`⏭️  ${sources[index]} sync skipped (lock not acquired)`);
          } else {
            console.error(`❌ ${sources[index]} sync failed:`, result.reason);
          }
        });
        
        return results;
      },
      300000 // 5 minute timeout for entire daily update
    );
    
    if (result === null) {
      console.log('⚠️  Daily grants update skipped - another instance is already running');
      process.exit(0);
    }
